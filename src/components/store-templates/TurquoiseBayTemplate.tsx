...
export default function TurquoiseBayTemplate({ store, products }: StoreTemplateProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-teal-50 to-blue-50 text-gray-900">
      <nav className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-lg' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            {store.logoUrl ? <img src={store.logoUrl} alt={store.name} className="h-12 w-12 rounded-full" /> : <div className="h-12 w-12 bg-gradient-to-br from-cyan-500 to-teal-600 rounded-full flex items-center justify-center font-bold text-xl text-white">{store.name[0]}</div>}
          ...
          <a href={`https://wa.me/${store.whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-700 hover:to-teal-700 text-white px-6 py-2 rounded-full font-semibold transition-all">Chat</a>
        </div>
      </nav>

      <header className="py-16 md:py-24 text-center px-4">
        <h2 className="text-4xl md:text-6xl font-black mb-4 text-transparent bg-gradient-to-r from-cyan-600 to-teal-600 bg-clip-text">{store.name}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product) => (
              <div key={product.id} className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
                <div className="relative aspect-square bg-gradient-to-br from-cyan-100 to-teal-100">
                  ...
                  {product.isNew && <span className="absolute top-3 right-3 bg-cyan-600 text-white text-xs px-3 py-1 rounded-full font-semibold">NEW</span>}
                ...
                <div className="p-5">
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-2xl font-bold text-cyan-600">Rp {product.price.toLocaleString('id-ID')}</p>
                  </div>
                  {product.hasConversionPage && product.conversionPageSlug ? (
                    <div className="space-y-2">
                      <Link href={`/p/${product.conversionPageSlug}`} className="block w-full bg-cyan-600 hover:bg-cyan-700 text-white py-2 rounded-lg font-semibold text-center transition-colors">Detail</Link>
                    ...
                    <button onClick={() => handleWhatsAppOrder(product.name, product.price)} className="w-full bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-700 hover:to-teal-700 text-white py-3 rounded-lg font-semibold transition-all">Pesan Sekarang</button>
                  )}
                </div>
              </div>
            ))}
      ...
      <StoreFooter store={store} bgColor="bg-cyan-100" textColor="text-gray-800" accentColor="text-cyan-900"/>
    </div>
  )
}
