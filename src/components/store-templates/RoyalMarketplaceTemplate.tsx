...
export default function RoyalMarketplaceTemplate({ store, products }: StoreTemplateProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-purple-950">
      ...
      {/* Royal Footer */}
      <footer className="relative border-t-2 border-yellow-400/30 mt-24">
        <div className="max-w-7xl mx-auto px-6 py-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            ...
            <div>
              <div className="flex items-center gap-3 mb-6">
                {store.logoUrl ? (
                  <img src={store.logoUrl} alt={store.name} className="h-16 w-16 rounded-full object-cover ring-4 ring-yellow-400" />
                ) : (
                  <div className="h-16 w-16 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-full flex items-center justify-center text-purple-900 font-black text-2xl ring-4 ring-yellow-400">
                    {store.name.charAt(0).toUpperCase()}
                  </div>
                )}
                <h3 className="text-2xl font-black text-transparent bg-gradient-to-r from-yellow-200 to-yellow-400 bg-clip-text">
                  {store.name}
                </h3>
              </div>
              {store.description && (
            ...
            {/* Contact */}
            <div>
              <h4 className="font-black text-yellow-400 text-lg mb-6 uppercase tracking-wider">Contact</h4>
            ...
            {/* Social */}
            <div>
              <h4 className="font-black text-yellow-400 text-lg mb-6 uppercase tracking-wider">Follow</h4>
              <div className="flex gap-4">
                {store.instagramUrl && (
                  <a href={store.instagramUrl} target="_blank" rel="noopener noreferrer" className="group relative bg-gradient-to-br from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 p-4 rounded-xl transition-all hover:scale-110 shadow-lg">
                ...
                {store.facebookUrl && (
                  <a href={store.facebookUrl} target="_blank" rel="noopener noreferrer" className="group relative bg-blue-600 hover:bg-blue-500 p-4 rounded-xl transition-all hover:scale-110 shadow-lg">
                ...
                {store.tiktokUrl && (
                  <a href={store.tiktokUrl} target="_blank" rel="noopener noreferrer" className="group relative bg-gray-800 hover:bg-gray-700 p-4 rounded-xl transition-all hover:scale-110 shadow-lg">
          ...
          <div className="border-t border-yellow-400/20 pt-8 text-center">
            <p className="text-purple-300 text-sm">© {new Date().getFullYear()} {store.name} - Royal Excellence</p>
            <div className="mt-2 text-xs text-purple-400">Crafted with Royal Precision</div>
          </div>
        </div>
      </footer>
    </div>
