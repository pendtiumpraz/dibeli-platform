...
export default function CoralReefTemplate({ store, products }: StoreTemplateProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-coral-50 to-red-50 text-gray-900">
      <nav className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-lg' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            {store.logoUrl ? <img src={store.logoUrl} alt={store.name} className="h-12 w-12 rounded-full" /> : <div className="h-12 w-12 bg-gradient-to-br from-orange-500 to-red-500 rounded-full flex items-center justify-center font-bold text-xl text-white">{store.name[0]}</div>}
          ...
          <a href={`https://wa.me/${store.whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white px-6 py-2 rounded-full font-semibold transition-all">Chat</a>
        </div>
      </nav>

      <header className="py-16 md:py-24 text-center px-4">
        <h2 className="text-4xl md:text-6xl font-black mb-4 text-transparent bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text">{store.name}</h2>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product) => (
              <div key={product.id} className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105" onMouseEnter={() => setHoveredProduct(product.id)} onMouseLeave={() => setHoveredProduct(null)}>
                <div className="relative aspect-square bg-gradient-to-br from-orange-100 to-red-100">
                  ...
                  {product.isNew && <span className="absolute top-3 right-3 bg-orange-600 text-white text-xs px-3 py-1 rounded-full font-semibold">NEW</span>}
                ...
                <div className="p-5">
                  ...
                  {product.hasConversionPage && product.conversionPageSlug ? (
                    ...
                    <button onClick={() => handleWhatsAppOrder(product.name, product.price)} className="w-full bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white py-3 rounded-lg font-semibold transition-all">Pesan Sekarang</button>
                  )}
                </div>
              </div>
            ))}
      ...
      <StoreFooter store={store} bgColor="bg-orange-100" textColor="text-gray-800" accentColor="text-orange-900"/>
    </div>
  )
}
