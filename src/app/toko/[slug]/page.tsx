...
export async function generateMetadata({ params }: StorePageProps): Promise<Metadata> {
  ...
  return {
    title: `${store.name} - ${store.tagline || 'Toko Online'} | dibeli.my.id`,
    description: store.description || `Belanja online di ${store.name}`,
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              <div key={product.id} className="bg-white rounded-lg shadow overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <a
                      href={generateWhatsAppLink(store.whatsappNumber, product.name, product.price)}
                      target="_blank"
      ...
      <footer className="bg-white border-t mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-center text-gray-600">
          <p>Powered by <span className="font-semibold text-purple-600">dibeli.my.id</span></p>
        </div>
      </footer>
    </div>
  )
